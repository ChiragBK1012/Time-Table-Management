    ...
    try {
        ...
        for (const slotData of slots) {
            try {
                ...
                if (
                    facultyClashScan.Items &&
                    facultyClashScan.Items.length > 0
                ) {
                    const clash = facultyClashScan.Items[0];
                    errors.push(
                ...
                errors.push(
                    `Error processing slot ${slotData.slot}: ${error.message}`
                );
            }
        }
    ...
    try {
        ...
        const result = await dynamoClient.query({
            ...
            KeyConditionExpression:
                "PK = :yearSection AND begins_with(SK, :day)",
            ExpressionAttributeValues: {
                ":yearSection": yearSection,
                ":day": `${day}#`,
            },
...
async function getNextClassForSubject(yearSection, subject) {
    try {
        ...
        for (const item of subjectSlots) {
            if (!item.day || !item.slot || !slotStartMinutes[item.slot])
                continue;
