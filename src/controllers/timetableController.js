...
async function addSingleSlot(req, res) {
    try {
        const { year_section, day, slot, subject, faculty, room, type } =
            req.body;
        ...
        for (const slot of slots) {
            if (
                ...
                return res.status(400).json({
                    success: false,
                    message:
                        "Each slot must have: slot, subject, faculty, room, type",
                });
            }
        }
    ...
    try {
        ...
        if (!year_section || !day || !slot) {
            return res.status(400).json({
                success: false,
                message:
                    "year_section, day, and slot query parameters are required",
            });
        }
    ...
    try {
        ...
        if (!faculty) {
            return res.status(400).json({
                success: false,
                message:
                    "Faculty name is required (use path param or ?faculty=)",
            });
        }
